...
const ViewRequestsView: React.FC<ViewRequestsViewProps> = ({ 
  ...
  const getVehicleInfo = (vehicleId?: string) => {
    const vehicle = vehicles.find(v => v.id === vehicleId);
    return vehicle ? `${vehicle.make} ${vehicle.model} - ${vehicle.plate}` : 'Sin asignar';
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
            ...
            <div>
              ...
              >
                ...
                <option value={RequestStatus.Pending}>Pendiente</option>
                <option value={RequestStatus.Approved}>Aprobada</option>
                <option value={RequestStatus.InProgress}>En Curso</option>
                <option value={RequestStatus.Completed}>Completada</option>
                <option value={RequestStatus.Canceled}>Cancelada</option>
              </select>
            </div>
...
const RequestDetailModal: React.FC<RequestDetailModalProps> = ({ request, vehicles, drivers, onClose }) => {
  const getVehicleInfo = (vehicleId?: string) => {
    const vehicle = vehicles.find(v => v.id === vehicleId);
    return vehicle ? `${vehicle.make} ${vehicle.model} - ${vehicle.plate}` : 'Sin asignar';
  };
  ...
  return (
    <Modal title="Detalle de Solicitud" onClose={onClose} isOpen={true}>
      <div className="space-y-4">
...
const AssignmentModal: React.FC<AssignmentModalProps> = ({ 
  ...
  return (
    <Modal title="Asignar Vehículo y Conductor" onClose={onClose} isOpen={true}>
      <div className="space-y-6">
            ...
            <div className="mt-2 p-3 bg-gray-50 rounded border border-gray-200">
              <div className="flex items-center text-sm">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    Contacto: {selectedDriverInfo.contact} • Licencia: {selectedDriverInfo.licenseNumber}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            <label className="flex items-start cursor-pointer">
              ...
              <div className="text-sm">
                ...
                <p className="text-gray-600">
                  Se notificará al solicitante ({request.requester}) y al conductor ({selectedDriverInfo?.name}) 
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          ...
          <button
            ...
            disabled={!selectedVehicle || !selectedDriver || !confirmAssignment}
            className={`px-6 py-2 rounded-lg font-medium transition-colors ${
              selectedVehicle && selectedDriver && confirmAssignment
