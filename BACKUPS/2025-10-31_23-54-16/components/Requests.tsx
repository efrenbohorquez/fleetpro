...
const Requests: React.FC<RequestsProps> = ({ requests, setRequests, vehicles, setVehicles, drivers, setDrivers, setSurveys }) => {
  ...
  const handleAssign = (vehicleId: string, driverId: string) => {
    if (selectedRequest) {
      // Si ya tenía vehículo/conductor asignado, liberarlos primero
      if (selectedRequest.vehicleId && selectedRequest.vehicleId !== vehicleId) {
        setVehicles(vehicles.map(v => 
          v.id === selectedRequest.vehicleId ? {...v, status: VehicleStatus.Available} : v
        ));
      }
      if (selectedRequest.driverId && selectedRequest.driverId !== driverId) {
        setDrivers(drivers.map(d => 
          d.id === selectedRequest.driverId ? {...d, status: DriverStatus.Available} : d
        ));
      }

      // Asignar nuevos vehículo y conductor
      setRequests(requests.map(r => 
        r.id === selectedRequest.id 
        ? { ...r, vehicleId, driverId, status: RequestStatus.InProgress } 
        : r
      ));
  ...
  return (
    <div>
              ...
              <tr key={req.id} className="bg-white border-b hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                   <div className="flex items-center justify-center space-x-2">
                        {/* Botón Asignar/Reasignar - disponible si no está completado o cancelado */}
                        {req.status !== RequestStatus.Completed && req.status !== RequestStatus.Canceled && (
                            <button 
                            onClick={() => handleOpenAssignModal(req)}
                            className="font-medium text-blue-600 hover:underline"
                            title={req.vehicleId ? "Reasignar Vehículo/Conductor" : "Asignar Vehículo/Conductor"}
                            >
                            {req.vehicleId ? "Reasignar" : "Asignar"}
                            </button>
                        )}
                         {/* Botón Completar - solo si está aprobado o en progreso */}
                         ...
                         {/* Botón Editar - siempre disponible excepto completados */}
                         ...
                         {/* Botón Eliminar - solo pendientes o cancelados */}
                         {(req.status === RequestStatus.Pending || req.status === RequestStatus.Canceled) && (
      ...
      {isAssignModalOpen && selectedRequest && <AssignModal request={selectedRequest} onAssign={handleAssign} onClose={() => setIsAssignModalOpen(false)} allVehicles={vehicles} allDrivers={drivers} />}
...
const AssignModal: React.FC<AssignModalProps> = ({ request, onClose, onAssign, allVehicles, allDrivers }) => {
    ...
    // Filtrar vehículos: disponibles + el actualmente asignado (si existe)
    const availableVehicles = allVehicles.filter(v => 
        v.status === VehicleStatus.Available || v.id === request.vehicleId
    );

    // Filtrar conductores: disponibles + el actualmente asignado (si existe)
    const availableDrivers = allDrivers.filter(d => 
        d.status === DriverStatus.Available || d.id === request.driverId
    );
    ...
    return (
        <Modal title={request.vehicleId ? "Reasignar Vehículo y Conductor" : "Asignar Vehículo y Conductor"} onClose={onClose}>
            <form onSubmit={handleSubmit}>
                {/* Header con información de solicitud */}
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    <div className="flex justify-between items-start">
                        <div>
                            <span className="text-blue-600 font-semibold">Solicitud N° {request.id}</span>
                            <p className="text-sm text-gray-600 mt-1">Solicitante: {request.requester}</p>
                            <p className="text-sm text-gray-600">Destino: {request.destination}</p>
                        </div>
                        {request.vehicleId && (
                            <div className="text-right text-sm">
                                <p className="text-orange-600 font-semibold">⚠️ Reasignación</p>
                                <p className="text-xs text-gray-500">Se liberarán recursos actuales</p>
                            </div>
                        )}
                    </div>
                </div>
        ...
        <Modal title={request ? 'Editar Solicitud' : 'Crear Solicitud'} onClose={onClose}>
            <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <input type="date" name="date" value={formData.date} onChange={handleChange} className="p-2 border rounded" title="Fecha de la solicitud" required />
